/**
    Purpose:
    - reads multiple PNG files, validates them, extracts their image data, decompresses it and vertically concatenates all the image data 
    into a single output images names all.png
    - Only PNGs with matching IHDR values (width, bit depth, color type, etc.) are accepted
    to ensure they can be combined into one image. 
    The result is a verticaly stacked image of the originals, compressed and writeen out as a valid PNG file
    
    Key Dependencies:
    - lab_png.h : handles PNG structures and parsing
    - zutil.h : for decompressing/compressing zlib streams
    - crc.h : for CRC32 checksum validation */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "lab_png.h"
...
#define PNG_SIG_SIZE 8 // standard PNG header size

int main(int argc, char *argv[]) { // If no input PNG files are specified, exit with usage info
    if (argc < 2) {
        fprintf(stderr, "Usage: %s [PNG_FILE]...\n", argv[0]);
        return EXIT_SUCCESS;
    }

    // Write the standard PNG signature to the output file
    ...
    struct data_IHDR ref_hdr; // Stores the first valid IHDR as a reference
    U8 *concat_data = NULL; // Buffer to accumulate deconpressed image data
    U64 concat_size = 0; // Total size of accumulated data
    U32 total_height = 0; // Combined image height
    int valid_pngs = 0; // Count of successfully processed PNG files

    // Loop through each PNG file provided
    for (int i = 1; i < argc; ++i) {
        ...
        // Check compatibility with first valid IHDR
        if (valid_pngs == 0) {
            ref_hdr = hdr; // Use this as the standard for all others
        ...
        U64 est_raw_size = hdr.height * (hdr.width * bpp + 1); // Overallocate to ensure space
        // Allocate buffer to decompress IDAT
        ...
        memcpy(concat_data + concat_size, raw, raw_len); // Append new data
        concat_size += raw_len;
        total_height += hdr.height;  // Increase total height
        valid_pngs++;
...
// If no valid PNGs, delete output and exit
    if (valid_pngs == 0) {
    ...
    // Construct IHDR for new output PNG
    ...
    // Compress the concatenated iamge data
